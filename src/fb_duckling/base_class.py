...
class BaseClass(object):
    ...
    def __init__(self, locale=default_locale, url=default_url, port=default_port, timezone=default_timezone, reftime=None):

        self.locale = locale or "en_US"
        self.url = url or "http://0.0.0.0"
        self.port = port or 8000
        self.timezone = timezone or "America/Los_Angeles"
        self.reftime = reftime if reftime else get_default_reftime()   # Unix epoch in milliseconds
